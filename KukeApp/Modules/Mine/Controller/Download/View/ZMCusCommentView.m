...
- (void)showView{
    [UIView animateWithDuration:0.2 animations:^{
        self.commentListView.frame = CGRectMake(0, ZMCusCommentViewTopHeight, SCREEN_WIDTH, SCREEN_HEIGHT-ZMCusCommentViewTopHeight);
        self.commentListView.videoInfo = self.videoInfo;
    } completion:^(BOOL finished) {
...
- (void)showCommentWithVideoInfo:(NSDictionary *)videoInfo{
...
//    [[[AppDelegate shareAppDelegate] getCurrentUIVC] addSubview:view];
    [[[AppDelegate shareAppDelegate] getCurrentUIVC].view addSubview:view];
    [view showView];
}
@end
