...
@interface SelectDownLoadTypeView ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SelectDownLoadTypeView
- (instancetype)initTovc:(UIViewController *)vc dataSource:(NSArray *)dataSource downLoadType:(NSString *)downLoadType{
    if (self = [super init]) {
        ...
        if ([downLoadType isEqualToString:@"标清"]) {
            self.currentIndex = 0;
        }else if ([downLoadType isEqualToString:@"超清"]){
            self.currentIndex = 2;
        }else{
            self.currentIndex = 1;
        }
...
-(UITableView *)tableView {
    if (!_tableView) {
        ...
        label.text = [NSString stringWithFormat:@"清晰度"];
        ...
        lab.text = [NSString stringWithFormat:@"请选择清晰度"];
