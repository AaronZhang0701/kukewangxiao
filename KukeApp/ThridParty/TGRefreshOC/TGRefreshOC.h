...
/** 进入刷新状态的回调 */
typedef void (^WaterDropRefreshingBlock)(void);
...
/** 正在刷新的回调 */
@property (copy, nonatomic) WaterDropRefreshingBlock refreshingBlock;

-(instancetype) initWithConfig:(void(^)(TGRefreshOC * refresh)) block;
/** 创建header */
+ (instancetype)headerWithRefreshingBlock:(WaterDropRefreshingBlock)refreshingBlock;
