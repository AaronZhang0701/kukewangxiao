...
- (void)setProgressView:(UIView *)progressView {
    ...
    if (_layout && _layout.barStyle == ZMPagerBarStyleCoverView) {
...
- (void)setDelegate:(id<ZMTabPagerBarDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(ZMTabPagerBarLayout *)layout {
...
- (ZMTabPagerBarLayout *)layout {
    if (!_layout) {
        _layout = [[ZMTabPagerBarLayout alloc]initWithPagerTabBar:self];
    }
    return _layout;
}
...
- (__kindof UICollectionViewCell<ZMTabPagerBarCellProtocol> *)dequeueReusableCellWithReuseIdentifier:(NSString *)identifier forIndex:(NSInteger)index {
    UICollectionViewCell<ZMTabPagerBarCellProtocol> *cell = [_collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    return cell;
}
...
- (UICollectionViewCell<ZMTabPagerBarCellProtocol> *)cellForIndex:(NSInteger)index
{
    if (index >= _countOfItems) {
        return nil;
    }
    return (UICollectionViewCell<ZMTabPagerBarCellProtocol> *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
}
...
{
    UICollectionViewCell<ZMTabPagerBarCellProtocol> *cell = [_dataSource pagerTabBar:self cellForItemAtIndex:indexPath.item];
...
{
    UICollectionViewCell<ZMTabPagerBarCellProtocol> *fromCell = [self cellForIndex:fromIndex];
    UICollectionViewCell<ZMTabPagerBarCellProtocol> *toCell = [self cellForIndex:toIndex];
