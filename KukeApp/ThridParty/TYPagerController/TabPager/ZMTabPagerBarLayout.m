...
@interface ZMTabPagerBarLayout ()

@property (nonatomic, weak) ZMTabPagerBar *pagerTabBar;
...
@implementation ZMTabPagerBarLayout

- (instancetype)initWithPagerTabBar:(ZMTabPagerBar *)pagerTabBar {
    if (self = [super init]) {
        _pagerTabBar = pagerTabBar;
        [self configurePropertys];
        self.barStyle = ZMPagerBarStyleProgressElasticView;
    }
    return self;
}
...
- (void)setProgressHeight:(CGFloat)progressHeight {
    ...
    frame.origin.y = _barStyle == ZMPagerBarStyleCoverView ? (height - _progressHeight)/2:(height - _progressHeight - _progressVerEdging);
    frame.size.height = progressHeight;
    _pagerTabBar.progressView.frame = frame;
}

- (UIEdgeInsets)sectionInset {
    if (!UIEdgeInsetsEqualToEdgeInsets(_sectionInset, UIEdgeInsetsZero) || _barStyle != ZMPagerBarStyleCoverView) {
        return _sectionInset;
    }
    if (_barStyle == ZMPagerBarStyleCoverView && _adjustContentCellsCenter) {
        return _sectionInset;
    }
...
- (void)setBarStyle:(ZMPagerBarStyle)barStyle
{
    if (barStyle == _barStyle) {
        return;
    }
    if (_barStyle == ZMPagerBarStyleCoverView) {
        self.progressBorderWidth = 0;
        self.progressBorderColor = nil;
    }
    _barStyle = barStyle;
    switch (barStyle) {
        case ZMPagerBarStyleProgressView:
            self.progressWidth = 0;
            self.progressHorEdging = 6;
            self.progressVerEdging = 0;
            self.progressHeight = kUnderLineViewHeight;
            break;
        case ZMPagerBarStyleProgressBounceView:
        case ZMPagerBarStyleProgressElasticView:
            self.progressWidth = 30;
            self.progressVerEdging = 0;
            self.progressHorEdging = 0;
            self.progressHeight = kUnderLineViewHeight;
            break;
        case ZMPagerBarStyleCoverView:
            self.progressWidth = 0;
    ...
    _pagerTabBar.progressView.hidden = barStyle == ZMPagerBarStyleNoneView;
    if (barStyle == ZMPagerBarStyleCoverView) {
        _progressRadius = 0;
...
- (void)transitionFromCell:(UICollectionViewCell<ZMTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<ZMTabPagerBarCellProtocol> *)toCell animate:(BOOL)animate {
...
- (void)transitionFromCell:(UICollectionViewCell<ZMTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<ZMTabPagerBarCellProtocol> *)toCell progress:(CGFloat)progress {
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    CGFloat progressY = _barStyle == ZMPagerBarStyleCoverView ? (cellFrame.size.height - _progressHeight)/2:(cellFrame.size.height - _progressHeight - _progressVerEdging);
...
- (void)setUnderLineFrameWithfromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    ...
    CGFloat progressY = _barStyle == ZMPagerBarStyleCoverView ? (toCellFrame.size.height - _progressHeight)/2:(toCellFrame.size.height - _progressHeight - _progressVerEdging);
    CGFloat progressX = 0, width = 0;
    
    if (_barStyle == ZMPagerBarStyleProgressBounceView) {
    ...
    }else if (_barStyle == ZMPagerBarStyleProgressElasticView) {
        if (fromCellFrame.origin.x < toCellFrame.origin.x) {
            if (progress <= 0.5) {
