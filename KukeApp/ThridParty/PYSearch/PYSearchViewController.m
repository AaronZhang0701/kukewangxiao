...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 8, screenWidth(), screenHeight()) style:UITableViewStyleGrouped];
...
{
    if (!_emptyLab) {
        UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(10, 20, 200, 40)];
        lab.text = @"暂无搜索纪录～";
        lab.font = [UIFont systemFontOfSize:13];
//        lab.hidden = YES;
        lab.textColor = CTitleColor;
        [self.searchHistoryView addSubview:lab];
        _emptyLab = lab;
    }
    return _emptyLab;
}
...
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"确定删除全部历史纪录吗？" preferredStyle:UIAlertControllerStyleAlert];
...
- (UILabel *)labelWithTitle:(NSString *)title
{
    ...
    label.backgroundColor = CBackgroundColor;
