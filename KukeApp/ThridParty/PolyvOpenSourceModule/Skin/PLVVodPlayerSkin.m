...
- (void)setDelegatePlayer:(PLVVodPlayerViewController *)delegatePlayer {
	...
	dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf.fullscreenView.backButton setTitle:@"" forState:UIControlStateNormal];
	});
}
...
//    }
    if ([self.is_hideNav isEqualToString:@"1"]) {
...
- (IBAction)backMainControl:(id)sender {
    ...
    [self fadeoutPlaybackControl];
//    [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30005] setHidden:YES];
//    [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30006] setHidden:YES];
//    BOOL isShowing = YES;
//    if (isShowing) {
//        [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30005] setHidden:YES];
//        [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30006] setHidden:YES];
//    }else{
//        [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30005] setHidden:NO];
//        [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30006] setHidden:NO];
//    }
//    isShowing = !isShowing;
...
-(void)downloadVideo{
    ...
    .LeeAddAction(^(LEEAction *action) {
        action.clickBlock = ^{
            [self downLoadType:(PLVVodQualityStandard)];
        };
        action.title = @"流畅";
    ...
    .LeeAddAction(^(LEEAction *action) {
        ...
        action.cornerRadius = 5.0f;
        action.clickBlock = ^{
            [self downLoadType:(PLVVodQualityHigh)];
        };
    })
    .LeeAddAction(^(LEEAction *action) {
        ...
        action.cornerRadius = 5.0f;
        action.clickBlock = ^{
            [self downLoadType:(PLVVodQualityUltra)];
        };
    })
    ...
    .LeeShow();
//    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"清晰度选择" message:nil preferredStyle:UIAlertControllerStyleAlert];
//
//    __weak typeof(self) weakSelf = self;
//
//    UIAlertAction *alertAction1 = [UIAlertAction actionWithTitle:@"流畅" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//        [weakSelf downLoadType:(PLVVodQualityStandard)];
//
//    }];
//
//    UIAlertAction *alertAction2 = [UIAlertAction actionWithTitle:@"高清" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//        [weakSelf downLoadType:(PLVVodQualityHigh)];
//    }];
//
//    UIAlertAction *alertAction3 = [UIAlertAction actionWithTitle:@"超清" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
//        [weakSelf downLoadType:(PLVVodQualityUltra)];
//    }];
//
//
...
- (void)downLoadType:(PLVVodQuality)type{
    
    
    PLVVodDownloadInfo * downLoadInfo = [[PLVVodDownloadManager sharedManager]requestDownloadInfoWithVid:self.videos.vid];
    [[PLVVodDownloadManager sharedManager] requestDownloadInfosWithCompletion:^(NSArray<PLVVodDownloadInfo *> *downloadInfos) {
        if ([downloadInfos containsObject:downLoadInfo]) {
...
//    if ([PLVVodDownloadManager videoExist:self.videos.vid] != 0) {
//        [BaseTools showErrorMessage:@"该课程已存在"];
//    }else{
//        PLVVodDownloadManager *downloadManager = [PLVVodDownloadManager sharedManager];
//
...
- (void)hideOrShowPlaybackControl {
	...
	[UIView animateWithDuration:PLVVodAnimationDuration animations:^{
        ...
        NSDictionary *dict =[[NSDictionary alloc]initWithObjectsAndKeys:[NSString stringWithFormat:@"%d",isShowing],@"isShowing", nil];
        KPostNotification(@"HiddenBackAndShareButton", dict);
        KPostNotification(@"HiddenCourseBackAndShareButton", dict);
//        UIInterfaceOrientation orientation = [UIApplication sharedApplication].statusBarOrientation;
//        if (UIInterfaceOrientationIsLandscape(orientation)){//横屏
//
//        }
//        else{
//            [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30005] setHidden:isShowing ? 0 : 1];
//            [[[[UIApplication sharedApplication] keyWindow] viewWithTag:30006] setHidden:isShowing ? 0 : 1];
//        }
//
//
