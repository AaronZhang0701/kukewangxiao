...
- (void)viewDidLoad {
    ...
    marquee.type = PLVMarqueeTypeFade;
    marquee.displayDuration = 2;
    marquee.timeIntervalConstant = YES;
    marquee.content = [NSString stringWithFormat:@"当前用户:%@",[UserDefaultsUtils valueWithKey:@"user_id"]];
    marquee.color = [UIColor colorWithHexString:@"#ffffff"];
    marquee.font = [UIFont systemFontOfSize:13];
    marquee.alpha = 0.8;
    ...
    if ([[UserDefaultsUtils valueWithKey:@"access_token"] length] != 0) {
        ...
        if ([BaseTools getCurrentNetworkState] == 0) {
            ...
            __weak typeof(self) weakSelf = self;
            [ZMNetworkHelper POST:@"/app/marquee" parameters:nil cache:YES responseCache:^(id responseCache) {
                ...
                if (responseObject == nil) {
                    ...
                    if ([responseObject[@"code"] isEqualToString:@"0"]) {
                        if ([responseObject[@"data"][@"marquee_switch"] isEqualToString:@"1"]) {
                            
                            dispatch_async(dispatch_get_main_queue(), ^{

                                marquee.maxFadeInterval = [responseObject[@"data"][@"ios_marquee_time"] integerValue];
                                marquee.maxRollInterval = [responseObject[@"data"][@"ios_marquee_time"] integerValue];
                                weakSelf.marquee = marquee;
                            });
...
#pragma mark - private
- (void)setLessonInfo:(NSDictionary *)lessonInfo{
    PLVVodPlayerSkin *skin = (PLVVodPlayerSkin *)self.playerControl;
    skin.lessonInfo = lessonInfo;
}
- (void)setupSkin {
	PLVVodPlayerSkin *skin = [[PLVVodPlayerSkin alloc] initWithNibName:nil bundle:nil];
    skin.is_hideNav = self.is_hideNav;
    skin.videoInfo = self.videoInfo;
