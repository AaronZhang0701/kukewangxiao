...
@property (nonatomic, strong) NSNotificationCenter *notificationDefaultCenter;
...
- (void)viewDidLoad {
    ...
    _notificationDefaultCenter = [NSNotificationCenter defaultCenter];
    //没有相册访问权限通知
    [_notificationDefaultCenter addObserver:self
...
- (void)dealloc {
    [_notificationDefaultCenter removeObserver:self];
    NSLog(@"%s",__func__);
}
...
- (void)setupNavBar {
    ...
    navBar.backgroundColor =CNavBgColor;
...
- (void)showNotAllowed {
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8.0) {
        alert = [[UIAlertView alloc] initWithTitle:@"提示"
                                           message:@"请在iPhone的“设置”-“隐私”-“照片”中，找到更改"
                                          delegate:nil
