...
-(void)setupTitles {
    ...
    btn.frame = CGRectMake(screenWidth()-screenWidth()/3, 0, screenWidth()/3, PageControllerTopBarHeight);
    [btn setTitle:@"库课公众号" forState:(UIControlStateNormal)];
    [btn setTitleColor:CTitleColor forState:(UIControlStateNormal)];
    btn.titleLabel.font = [UIFont systemFontOfSize:15];
    [btn addTarget:self action:@selector(btnClick) forControlEvents:(UIControlEventTouchUpInside)];
    [self.titleCollectionView addSubview:btn];
    
    
}
- (void)btnClick{
    KPostNotification(@"kukegongzhonghao", nil);
}
...
- (void)setTitleItemWidth {
    if (self.titles.count < 4) {
        _titleItemWidth = (kWidth/3*2-10)/(self.titles.count);
    }else {
        _titleItemWidth = (kWidth/3*2-10)/4;
