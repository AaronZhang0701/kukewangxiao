...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.avatarView = [[UIImageView alloc] initWithFrame:CGRectMake(screenWidth()-45, 10, 35, 35)];
        ...
        self.messageLB.textColor = CTitleColor;
        self.messageLB.font = CHAT_TEXT_FONT;
        self.messageLB.backgroundColor = [UIColor colorWithRed:178/255.0 green:228/255.0 blue:111/255.0 alpha:1];
        ...
        if ([UserInfoTool persons].count != 0) {
            ...
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:info.photo] placeholderImage:[UIImage imageNamed:@"个人中心未登录头像"]];
            if (info.Tel.length == 11) {
                NSString *numberString = [info.Tel encryptionUserName];
                self.nickNameLB.text = numberString;
            }else{
                self.nickNameLB.text = @"游客";
            }
        }else{
            self.avatarView.image = [UIImage imageNamed:@"个人中心未登录头像"];
        ...
        [self.nickNameLB mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.trailing.equalTo(self.avatarView.mas_leading).offset(-10);
        }];
    }
    return self;
}
...
- (CGFloat)calculateCellHeightWithContent:(NSString *)content {
    ...
    return [self autoCalculateSize:CGSizeMake(260, MAXFLOAT) attributedContent:attributedStr].height + 60.0;
}

@end
#pragma mark - 其他学员发言的cell
