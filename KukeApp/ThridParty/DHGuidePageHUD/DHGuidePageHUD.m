...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        [skipButton addTarget:self action:@selector(buttonClick2:) forControlEvents:UIControlEventTouchUpInside];
...
//                [imageView addSubview:startButton];
                ...
                // 视频引导页进入按钮
                UIButton *movieStartButton = [[UIButton alloc] initWithFrame:CGRectMake(screenWidth()/2-139-17, screenHeight()-150, 139, 47)];
                [movieStartButton setBackgroundImage:[UIImage imageNamed:@"按钮渐变背景"] forState:(UIControlStateNormal)];
                [movieStartButton setTitle:@"新人体验" forState:UIControlStateNormal];
                [movieStartButton.titleLabel setFont:[UIFont systemFontOfSize:17]];
                [imageView addSubview:movieStartButton];
                [movieStartButton addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];

                
                UIButton *movieStartButton1 = [[UIButton alloc] initWithFrame:CGRectMake(screenWidth()/2+17, screenHeight()-150, 139, 47)];
                [movieStartButton1.layer setBorderWidth:1.0];
                [movieStartButton1.layer setCornerRadius:23.5];
                [movieStartButton1.layer setBorderColor:[CNavBgColor CGColor]];
                [movieStartButton1.titleLabel setFont:[UIFont systemFontOfSize:17]];
                [movieStartButton1 setTitleColor:CNavBgColor forState:(UIControlStateNormal)];
                [movieStartButton1 setTitle:@"已购课登录" forState:UIControlStateNormal];
                [imageView addSubview:movieStartButton1];
                [movieStartButton1 addTarget:self action:@selector(buttonClick1:) forControlEvents:UIControlEventTouchUpInside];
        ...
        self.imagePageControl.pageIndicatorTintColor = [UIColor colorWithHexString:@"#F29094"];
        self.imagePageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#E62129"];
...
- (instancetype)dh_initWithFrame:(CGRect)frame videoURL:(NSURL *)videoURL {
    if ([super initWithFrame:frame]) {
        ...
        // 视频引导页进入按钮
        UIButton *movieStartButton = [[UIButton alloc] initWithFrame:CGRectMake(screenWidth()/2-139-17, screenHeight()-85, 139, 47)];
...
//        [movieStartButton.layer setBorderColor:[UIColor whiteColor].CGColor];
        ...
        UIButton *movieStartButton1 = [[UIButton alloc] initWithFrame:CGRectMake(screenWidth()/2+17, screenHeight()-85, 139, 47)];
        [movieStartButton1.layer setBorderWidth:1.0];
        [movieStartButton1.layer setCornerRadius:20.0];
        [movieStartButton1.layer setBorderColor:[UIColor whiteColor].CGColor];
        [movieStartButton1 setTitle:@"已购课登录" forState:UIControlStateNormal];
        [movieStartButton1 setAlpha:0.0];
        [self.playerController.view addSubview:movieStartButton1];
        [movieStartButton1 addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
        [UIView animateWithDuration:DDHidden_TIME animations:^{
            [movieStartButton1 setAlpha:1.0];
        }];
