...
#import "ZMTimeCountDown.h"


@interface ZMTimeCountDown()
...
@implementation ZMTimeCountDown


+(instancetype)ShareManager{
    static dispatch_once_t onceToken;
    static ZMTimeCountDown *manager = nil;
    dispatch_once(&onceToken, ^{
        manager = [[ZMTimeCountDown alloc]init];
    });
    return manager;
}
...
// 倒计时  时分秒
-(void)zj_timeCountDownHMSWithStartTimeStamp:(long long)startTimeStamp endTimeStamp:(long long)endTimeStamp completeBlock:(ZJTimeDownCompleteBlock)completeBlock{
    if (self.timer == nil) {
        NSDate *finishDate = [self zj_timeDateWithLongLong:endTimeStamp];
        NSDate *startDate  = [self zj_timeDateWithLongLong:startTimeStamp];
        NSTimeInterval timeInterval =[finishDate timeIntervalSinceDate:startDate];
