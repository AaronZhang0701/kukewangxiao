    ...
    [ZMNetworkHelper POST:@"/app/app_start_up" parameters:parmDic cache:NO responseCache:^(id responseCache) {
        ...
        if ([responseObject[@"code"] isEqualToString:@"0"]) {
            ...
            for (NSDictionary *dict in responseObject[@"data"]) {
                [ary addObject:dict[@"image"]];
                [urlAry addObject:dict[@"url"]];

            }
            NSArray *stringArr = [ary[0] componentsSeparatedByString:@"/"];
            NSString *imageName = stringArr.lastObject;
            [kUserDefaults setValue:urlAry[0] forKey:adUrl];
            // 拼接沙盒路径
            NSString *filePath = [self getFilePathWithImageName:imageName];
            BOOL isExist = [self isFileExistWithFilePath:filePath];
            if (!isExist){// 如果该图片不存在，则删除老图片，下载新图片

                [self downloadAdImageWithUrl:ary[0] imageName:imageName adUrl:urlAry[0]];
 ...
 *  下载新图片
 */
- (void)downloadAdImageWithUrl:(NSString *)imageUrl imageName:(NSString *)imageName adUrl:(NSString *)url
{
