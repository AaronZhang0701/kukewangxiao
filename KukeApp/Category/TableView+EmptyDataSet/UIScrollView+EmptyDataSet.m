...
- (id<ZMEmptyDataSetSource>)emptyDataSetSource
{
    return objc_getAssociatedObject(self, kEmptyDataSetSource);
}

- (id<ZMEmptyDataSetDelegate>)emptyDataSetDelegate
{
...
- (BOOL)dzn_canDisplay
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource conformsToProtocol:@protocol(ZMEmptyDataSetSource)]) {
...
- (void)setEmptyDataSetSource:(id<ZMEmptyDataSetSource>)datasource
{
...
- (void)setEmptyDataSetDelegate:(id<ZMEmptyDataSetDelegate>)delegate
{
    if (!delegate) {
        [self dzn_invalidate];
    }
...
#pragma mark - UIView+ZMConstraintBasedLayoutExtensions
